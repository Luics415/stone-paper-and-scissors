...
// ==========================================
// GENERADOR ADAPTATIVO DE TEXTOS EN PANTALLA
// ==========================================
function spawnBackgroundTaunt(messagesArray, typeClass) {
  ...
  const isMobile = window.innerWidth <= 768; // Detecta si es teléfono

  if (isMobile) {
    // EN TELÉFONO: Se centra horizontalmente y aparece arriba o abajo de la tarjeta
    tauntEl.style.left = "50%"; // Centrado mediante CSS transform
    
    // 50% de probabilidad de salir arriba del título o abajo de los botones
    const spawnOnTop = Math.random() < 0.5;
    if (spawnOnTop) {
      tauntEl.style.top = "5%";
    } else {
      tauntEl.style.bottom = "8%";
    }
  } else {
    // EN PC / TABLET: Aparece a los lados
    const isLeft = Math.random() < 0.5;
    if (isLeft) {
      tauntEl.style.left = `${Math.random() * 12 + 5}%`;
    } else {
      tauntEl.style.left = `${Math.random() * 12 + 83}%`;
    }
  ...
  // Limpiar nodo del DOM tras finalizar la animación (3.5s)
  setTimeout(() => {
    tauntEl.remove();
  }, 3500);
}
